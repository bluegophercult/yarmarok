...
export const useDonationStore = defineStore({
    ...
    actions: {
        async getDonations(raffleId: string, prizeId: string, ticketCost: number) {
            ...
            this.donations.forEach((d, i) =>
                this.donations[i].ticketsNumber = Math.floor(d.amount / ticketCost),
            )
